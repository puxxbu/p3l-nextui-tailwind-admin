...
const CetakLaporanEmpat = () => {
  ...
  const { status, data, error, refetch, isLoading } = useQuery(
    ['laporanEmpatCetak'], // Memasukkan filterValue sebagai bagian dari query key
    () =>
      fetchLaporanEmpat(
        parseInt(tahun || '2023'),
  ...
  return (
    <DefaultLayout>
      <Breadcrumb pageName="Laporan 5 Customer Reservasi Terbanyak" />
      ...
      >
        ...
        <table className="mb-8 w-full  border-black text-left  text-gray-700 dark:border-gray-300 dark:text-white">
          ...
          <tbody>
            {data?.data.topCustomers.map((item: any, index: any) => (
              <tr key={index}>
                ...
                <td className="border-2 border-black px-2 py-3 font-semibold dark:border-gray-300">
                  {item.total_pembayaran.toLocaleString('id-ID')}
                </td>
              </tr>
            ))}
            {/* <tr>
...
export default CetakLaporanEmpat;
